...
def test_hll_merge():
    hll_total = khmer.new_hll_counter(ERR_RATE, K)
    hll_merged = khmer.new_hll_counter(ERR_RATE, K)

    filename = utils.get_test_data("test-abund-read-2.fa")
    hll_partial_1 = khmer.new_hll_counter(ERR_RATE, K)
    hll_partial_1.consume_fasta(filename)
    hll_total.consume_fasta(filename)

    filename = utils.get_test_data("test-abund-read-3.fa")
    hll_partial_2 = khmer.new_hll_counter(ERR_RATE, K)
